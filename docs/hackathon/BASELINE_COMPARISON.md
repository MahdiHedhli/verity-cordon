...
| Shadow behavior | A permissive policy can detect and allow, but there is no explicit shadow mode or actual versus would-have action pair. | Explicit enforce/shadow modes with both actions, policy version, provider state, and later rescan/revoke. |
