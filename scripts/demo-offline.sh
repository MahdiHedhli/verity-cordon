...
DEMO_DIR="${VERITY_DEMO_DATA_DIR:-$ROOT_DIR/.verity-demo}"
if [[ "$DEMO_DIR" == "$ROOT_DIR/.verity-demo" ]]; then
  rm -rf -- "$DEMO_DIR"
fi

export VERITY_DATA_DIR="$DEMO_DIR"
export VERITY_SEMANTIC_PROVIDER="fixture"
...
if [[ "${VERITY_DEMO_NO_SERVE:-0}" == "1" ]]; then
  uv run verity demo offline --no-serve
else
  ...
  uv run verity demo offline --serve
fi
